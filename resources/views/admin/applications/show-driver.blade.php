<x-layouts.admin>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden" x-data="{ showRejectModal: false }">
            ...
            <div class="relative bg-gradient-to-r from-luffy-red/20 to-rose-500/10 p-8 flex flex-col sm:flex-row items-center gap-6 justify-between border-b border-luffy-red/30 transition-colors duration-300">
                <div class="flex items-center gap-6">
                    <div class="w-20 h-20 rounded-3xl bg-white dark:bg-slate-900 border-2 border-luffy-red/20 flex items-center justify-center font-extrabold text-3xl text-luffy-red shadow-md overflow-hidden font-['Outfit']">
                    ...
                    <div>
                        @if ($driverProfile->user)
                            <h1 class="text-2xl font-extrabold font-['Outfit'] text-slate-800 dark:text-white">{{ $driverProfile->user->first_name }} {{ $driverProfile->user->last_name }}</h1>
                            <p class="text-xs text-slate-450 dark:text-slate-400 mt-1.5 font-medium">Registrado como postulante el {{ $driverProfile->created_at->format('d/m/Y H:i') }}</p>
                        @else
                            <h1 class="text-2xl font-extrabold font-['Outfit'] text-slate-850 dark:text-white">Repartidor sin Usuario</h1>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    @if ($driverProfile->status === 'pending')
                        <span class="px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider bg-amber-500 text-white shadow-lg shadow-amber-500/20 animate-pulse">Pendiente</span>
            ...
            <div class="p-8">
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-400">Datos Personales</h3>
                            
                            @if ($driverProfile->user)
                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-450 block font-bold">DNI / Documento</span>
                                    <span class="text-sm font-mono font-bold text-slate-800 dark:text-slate-200">{{ $driverProfile->user->dni ?? 'No especificado' }}</span>
                                </div>

                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-450 block font-bold">Teléfono Móvil</span>
                                    <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ $driverProfile->user->phone ?? 'No especificado' }}</span>
                                </div>

                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-450 block font-bold">Correo Electrónico</span>
                                    <a href="mailto:{{ $driverProfile->user->email }}" class="text-sm font-bold text-luffy-red dark:text-rose-450 hover:underline block">{{ $driverProfile->user->email }}</a>
                                </div>
                            @else
                                <div class="text-slate-450 dark:text-slate-500 text-sm py-2 italic font-medium">No se encontró información personal.</div>
                            @endif
                        </div>

                        <!-- Columna de Datos de Vehículo -->
                        <div class="space-y-4 md:border-l border-slate-150/40 dark:border-slate-700/50 md:pl-6">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-400">Datos del Vehículo</h3>
                            
                            <div>
                                <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Tipo de Vehículo</span>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-black bg-slate-100 dark:bg-slate-700/60 text-slate-800 dark:text-slate-200 mt-1.5 uppercase border border-slate-200/40 dark:border-slate-600/30">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Marca / Modelo</span>
                                    <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ $driverProfile->vehicle_brand ?? 'N/A' }} {{ $driverProfile->vehicle_model ?? 'N/A' }}</span>
                                </div>
                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Color</span>
                                    <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ $driverProfile->vehicle_color ?? 'N/A' }}</span>
                                </div>
                            </div>

                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Placa</span>
                                ...
                                <div>
                                    <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">N° de Licencia</span>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-700/60 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-400">Contacto de Emergencia</h3>

                            <div>
                                <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Nombre de Contacto</span>
                            ...
                            <div>
                                <span class="text-xs text-slate-400 dark:text-slate-455 block font-bold">Teléfono de Contacto</span>
                                <span class="text-sm font-bold text-slate-800 dark:text-slate-200 block">{{ $driverProfile->emergency_contact_phone ?? 'No especificado' }}</span>
                            </div>
                        </div>

                        <div class="space-y-4 md:border-l border-slate-150/40 dark:border-slate-700/50 md:pl-6">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-400">Preferencia de Pagos</h3>

                            <div>
                                <span class="text-xs text-slate-400 dark:text-slate-450 block font-bold mb-1.5">Acepta Pagos en Efectivo</span>
                                @if ($driverProfile->accepts_cash_payments)
                                    <span class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 px-3 py-1.5 rounded-xl border border-emerald-100/30 dark:border-emerald-900/20">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-950/20 px-3 py-1.5 rounded-xl border border-rose-100/30 dark:border-rose-900/20">
                                        <svg class="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>
                        ...
                        <div class="pt-6 border-t border-slate-100 dark:border-slate-700/60">
                            ...
                            @if ($driverProfile->status === 'active')
                                <div class="p-4 bg-emerald-50/50 dark:bg-emerald-950/15 border border-emerald-100 dark:border-emerald-900/40 rounded-2xl">
                                    ...
                                    <p class="text-xs text-slate-550 dark:text-slate-400 mt-1 font-medium">
                            ...
                            @elseif ($driverProfile->status === 'rejected')
                                <div class="p-4 bg-rose-50/50 dark:bg-rose-950/15 border border-rose-100 dark:border-rose-900/40 rounded-2xl space-y-2">
                                    ...
                                    <div>
                                        <span class="text-xs text-slate-400 dark:text-slate-405 block font-bold">Motivo del rechazo:</span>
            ...
            <div x-show="showRejectModal" 
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 w-full max-w-lg p-6 shadow-2xl relative"
                    ...
                    <button type="button" @click="showRejectModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 cursor-pointer">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>

                    <h2 class="text-xl font-extrabold font-['Outfit'] text-slate-850 dark:text-white">Rechazar Solicitud de Repartidor</h2>
