    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden" x-data="{ showRejectModal: false }">
            ...
            <div class="relative h-48 bg-slate-900 flex items-center justify-center overflow-hidden">
                ...
                @else
                    <div class="absolute inset-0 bg-gradient-to-r from-slate-950 to-slate-850 opacity-90"></div>
                    <div class="absolute text-slate-700 text-sm font-semibold uppercase tracking-wider">Sin imagen de portada</div>
                @endif

                <!-- Badge de Estado -->
                <div class="absolute top-6 right-6">
                    @if ($business->status === 'pending')
                        <span class="px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider bg-amber-500 text-white shadow-lg shadow-amber-500/20">Pendiente de Revisión</span>
            ...
            <div class="p-8 relative">
                ...
                <div class="pt-8 space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-400 mt-1">Registrado el {{ $business->created_at->format('d \d\e F, Y \a \l\a\s H:i') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                        <div class="space-y-4">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400">Datos Comerciales</h3>
                            
                            <div>
                                <span class="text-xs text-slate-400 block">Razón Social</span>
                                <span class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ $business->legal_name ?? 'No especificado' }}</span>
                            </div>

                            <div>
                                <span class="text-xs text-slate-400 block">Número RUC</span>
                                <span class="text-sm font-mono font-bold text-slate-800 dark:text-slate-200">{{ $business->ruc ?? 'No especificado' }}</span>
                            </div>

                            <div>
                                <span class="text-xs text-slate-400 block">Categorías</span>
                                <div class="flex flex-wrap gap-1.5 mt-1">
                                    @forelse ($business->categories as $category)
                                        <span class="px-2.5 py-1 text-xs font-medium rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">{{ $category->name }}</span>
                                    @empty
                                        <span class="text-xs text-slate-500">Ninguna categoría asignada</span>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-400">Datos de Contacto del Negocio</h3>

                            <div>
                                <span class="text-xs text-slate-400 block">Correo Electrónico</span>
                                <a href="mailto:{{ $business->contact_email }}" class="text-sm font-semibold text-luffy-red hover:underline block">{{ $business->contact_email ?? 'No especificado' }}</a>
                            </div>

                            <div>
                                <span class="text-xs text-slate-400 block">Teléfono de Contacto</span>
                                <span class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ $business->contact_phone ?? 'No especificado' }}</span>
                            </div>

                            <div>
                                <span class="text-xs text-slate-400 block">WhatsApp de Pedidos</span>
                                @if ($business->whatsapp_number)
                                    <a href="https://wa.me/{{ preg_replace('/\D/', '', $business->whatsapp_number) }}" target="_blank" class="text-sm font-semibold text-emerald-600 hover:underline flex items-center gap-1">
                                ...
                                @else
                                    <span class="text-sm text-slate-500">No especificado</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 dark:border-slate-700">
                        <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 mb-2">Descripción del Negocio</h3>
                        <p class="text-slate-600 dark:text-slate-300 text-sm leading-relaxed bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="pt-4 border-t border-slate-100 dark:border-slate-700">
                        <h3 class="text-xs font-black uppercase tracking-wider text-slate-400 mb-4">Información del Propietario</h3>
                        <div class="bg-slate-50 dark:bg-slate-900/55 rounded-3xl p-6 border border-slate-100 dark:border-slate-800/80 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                            @if ($owner)
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-tr from-luffy-red to-rose-500 text-white flex items-center justify-center font-bold text-lg shadow-md shadow-luffy-red/10">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-xs text-slate-400 block">DNI: {{ $owner->dni ?? 'No especificado' }}</span>
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm w-full md:w-auto">
                                    <div>
                                        <span class="text-xs text-slate-400 block">Correo Electrónico</span>
                                        <a href="mailto:{{ $owner->email }}" class="font-semibold text-slate-700 dark:text-slate-300 hover:underline">{{ $owner->email }}</a>
                                    </div>
                                    <div>
                                        <span class="text-xs text-slate-400 block">Teléfono Personal</span>
                                        <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $owner->phone ?? 'No especificado' }}</span>
                                    </div>
                                </div>
                            @else
                                <div class="text-slate-400 text-sm py-2">No se encontró información del usuario propietario.</div>
                            @endif
                        </div>
                    </div>
            ...
            <div x-show="showRejectModal" 
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 w-full max-w-lg p-6 shadow-2xl relative"
                    ...
                    <h2 class="text-xl font-extrabold font-['Outfit'] text-slate-800 dark:text-white">Rechazar Solicitud de Negocio</h2>
