<x-layouts.admin>
    <x-slot:title>Nueva Categoría - Nikama Admin</x-slot:title>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden"
             x-data="{ 
                name: '{{ old('name') }}', 
                slug: '{{ old('slug') }}', 
                manualSlug: {{ old('slug') ? 'true' : 'false' }},
                generateSlug() {
                    if (!this.manualSlug) {
                        this.slug = this.name
                            .toLowerCase()
            ...
            <div class="p-6 border-b border-slate-100 dark:border-slate-700">
                <h3 class="text-xl font-extrabold font-['Outfit'] text-slate-800 dark:text-white">Crear Nueva Categoría</h3>
                <p class="text-xs text-slate-400 mt-1">Registra un nuevo sector comercial o una subcategoría en el árbol jerárquico.</p>
            </div>
            ...
            <form action="{{ route('admin.categories.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <span class="text-slate-400 text-[10px] block mt-1">Generado automáticamente. Modifícalo si deseas personalizarlo.</span>
                        @error('slug')
                            <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <span class="text-slate-400 text-[10px] block mt-1">Selecciona una categoría padre si deseas crear una subcategoría. De lo contrario, se considerará sector raíz.</span>
                        @error('parent_id')
                            <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon') }}" placeholder="Ej: 🍔, 💊, 🍺"
                               class="w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">
                    ...
                    <div>
                        ...
                        <input type="url" name="image_url" id="image_url" value="{{ old('image_url') }}" placeholder="https://ejemplo.com/imagen.jpg"
                               class="w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" placeholder="Describe brevemente el alcance de esta categoría..."
                              class="w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
