<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>{{ $title ?? 'Driver Dashboard - Nikama' }}</title>
    <!-- Fonts -->
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=inter:400,500,600,700|outfit:400,600,700&display=swap" rel="stylesheet" />

    <!-- Leaflet CSS for Map -->
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" integrity="sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=" crossorigin="" />
    <!-- Leaflet JS for Map -->
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" integrity="sha256-20nQCchB9co0qIjJZRGuk2/Z9VM+kNiyxNV1lvTlZBo=" crossorigin=""></script>

    <!-- Scripts -->
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
    <script>
        if (localStorage.theme === 'dark' || (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
            document.documentElement.classList.add('dark');
        } else {
            document.documentElement.classList.remove('dark');
        }
    </script>
</head>
<body class="antialiased font-sans bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-white transition-colors duration-300" x-data="{ sidebarOpen: false }">
    <div class="flex h-screen overflow-hidden">
        @include('components.layouts.driver.sidebar')

        <div class="flex flex-col flex-1 overflow-hidden">
            @include('components.layouts.driver.topbar')

            <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-8">
                {{ $slot }}
            </main>
        </div>
    </div>

    <!-- Driver Notification Toast Container -->
    <div id="driver-notification-toast-container" class="fixed bottom-5 right-5 z-[9999] flex flex-col gap-3 max-w-sm w-full pointer-events-none px-4 sm:px-0"></div>

    <script>
        document.addEventListener('DOMContentLoaded', () => {
            let lastPendingCount = null;
            const checkUrl = "{{ route('driver.assignments.pending-count') }}";
            const dashboardUrl = "{{ route('driver.dashboard') }}";

            // --- Audio: Mixkit CDN + AudioContext Fallback ---
            const MIXKIT_ALARM_URL = 'https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3';
            let mixkitAudio = null;

            function loadMixkitAudio() {
                try {
                    mixkitAudio = new Audio(MIXKIT_ALARM_URL);
                    mixkitAudio.volume = 0.7;
                    mixkitAudio.load();
                } catch (e) {
                    mixkitAudio = null;
                }
            }

            function playAlarmSound() {
                const REPEAT = 3;
                const GAP_MS = 2500;

                function playSingleMixkit(attempt) {
                    if (!mixkitAudio) { playSingleFallback(attempt); return; }
                    mixkitAudio.currentTime = 0;
                    mixkitAudio.play().catch(() => playSingleFallback(attempt));
                    if (attempt < REPEAT) {
                        setTimeout(() => playSingleMixkit(attempt + 1), GAP_MS);
                    }
                }

                function playSingleFallback(attempt) {
                    playFallbackAlarm();
                    if (attempt < REPEAT) {
                        setTimeout(() => playSingleFallback(attempt + 1), GAP_MS);
                    }
                }

                if (mixkitAudio) {
                    playSingleMixkit(1);
                } else {
                    playSingleFallback(1);
                }
            }

            function playFallbackAlarm() {
                try {
                    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
                    let time = audioCtx.currentTime;

                    // Urgent 3-tone alarm pattern
                    const tones = [880, 1100, 880, 1100, 880];
                    tones.forEach((freq, i) => {
                        const osc = audioCtx.createOscillator();
                        const gain = audioCtx.createGain();

                        osc.type = 'square';
                        osc.frequency.setValueAtTime(freq, time);

                        gain.gain.setValueAtTime(0, time);
                        gain.gain.linearRampToValueAtTime(0.15, time + 0.02);
                        gain.gain.exponentialRampToValueAtTime(0.001, time + 0.28);

                        osc.connect(gain);
                        gain.connect(audioCtx.destination);
                        osc.start(time);
                        osc.stop(time + 0.3);
                        time += 0.35;
                    });
                } catch (e) {
                    console.warn('Driver alarm AudioContext failed:', e);
                }
            }

            function updateTopbarBadge(count) {
                const badge = document.getElementById('driver-topbar-notification-badge');
                if (badge) {
                    if (count > 0) {
                        badge.textContent = count;
                        badge.style.display = 'flex';
                    } else {
                        badge.style.display = 'none';
                    }
                }
            }

            function showDriverToast(assignment) {
                const container = document.getElementById('driver-notification-toast-container');
                if (!container) return;

                const total = parseFloat(assignment.total || 0).toFixed(2);
                const isEven = container.children.length % 2 === 0;

                const toast = document.createElement('div');
                toast.className = 'pointer-events-auto w-full bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border border-rose-400/30 rounded-3xl shadow-2xl p-4 transform translate-y-4 opacity-0 transition-all duration-300 flex items-start gap-3';

                toast.innerHTML = `
                    <div class="relative flex-shrink-0 w-11 h-11 bg-rose-500/10 rounded-2xl flex items-center justify-center text-rose-500">
                        <span class="absolute -top-1 -right-1 flex h-3 w-3">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-3 w-3 bg-rose-500"></span>
                        </span>
                        <svg class="w-5 h-5 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                        </svg>
                    </div>
                    <div class="flex-1 min-w-0 space-y-1">
                        <h4 class="font-extrabold text-sm text-slate-800 dark:text-white font-['Outfit']">🚨 ¡Nuevo Pedido Asignado!</h4>
                        <p class="text-[11px] text-slate-600 dark:text-slate-400 font-semibold truncate">
                            <span class="text-rose-500 font-black">${assignment.order_number}</span> — ${assignment.business_name}
                        </p>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400 truncate">📍 ${assignment.delivery_address}</p>
                        <div class="flex items-center gap-2 pt-1.5">
                            <a href="${dashboardUrl}"
                               class="inline-flex items-center gap-1 bg-rose-500 hover:bg-rose-600 text-white text-[11px] font-black uppercase tracking-wider py-1.5 px-3 rounded-xl transition cursor-pointer">
                                Ver Pedido ➜
                            </a>
                            <span class="text-[10px] font-black text-emerald-600 dark:text-emerald-400">S/ ${total}</span>
                        </div>
                    </div>
                    <button class="text-slate-400 hover:text-slate-600 dark:hover:text-white transition flex-shrink-0" onclick="this.parentElement.remove()">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                `;

                container.appendChild(toast);

                // Animate in
                setTimeout(() => {
                    toast.classList.remove('translate-y-4', 'opacity-0');
                }, 10);

                // Auto-remove after 15 seconds
                setTimeout(() => {
                    if (toast && toast.parentElement) {
                        toast.classList.add('opacity-0', 'translate-y-4');
                        setTimeout(() => toast.remove(), 300);
                    }
                }, 15000);
            }

            function pollDriverAssignments() {
                fetch(checkUrl)
                    .then(res => res.json())
                    .then(data => {
                        const count = data.count;
                        updateTopbarBadge(count);

                        if (lastPendingCount !== null && count > lastPendingCount) {
                            playAlarmSound();
                            // Show one toast per new assignment
                            const newAssignments = data.assignments.slice(0, count - lastPendingCount);
                            newAssignments.forEach(assignment => showDriverToast(assignment));
                        }

                        lastPendingCount = count;
                    })
                    .catch(err => console.error('Driver assignment polling error:', err));
            }

            // Preload audio
            loadMixkitAudio();

            // Initial silent load to set baseline count
            fetch(checkUrl)
                .then(res => res.json())
                .then(data => {
                    lastPendingCount = data.count;
                    updateTopbarBadge(data.count);
                    // Start polling every 5 seconds
                    setInterval(pollDriverAssignments, 5000);
                })
                .catch(err => console.error('Driver initial poll error:', err));
        });
    </script>
</body>
</html>
