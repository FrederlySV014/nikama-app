<x-layouts.public>
    <x-slot:title>Editar Dirección - Nikama</x-slot:title>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8" x-data="addressForm()">
        ...
        <nav class="flex text-xs font-semibold text-slate-500 dark:text-slate-400 mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1.5 md:space-x-2">
                ...
                <li class="flex items-center">
                    <svg class="w-3 h-3 text-slate-400 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                    <span class="text-slate-800 dark:text-slate-200 font-bold" aria-current="page">Editar Dirección</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/80 p-6 sm:p-8 shadow-sm">
                <div class="mb-6 pb-6 border-b border-slate-100 dark:border-slate-700/50">
                    <h1 class="text-2xl font-extrabold font-['Outfit'] text-slate-900 dark:text-white">Editar Dirección</h1>
                    <p class="text-sm text-slate-550 dark:text-slate-400 mt-1">Modifica los detalles de tu dirección guardada.</p>
                </div>

                <form action="{{ route('profile.addresses.update', $address) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input x-model="form.label" name="label" type="text" required
                        ...
                        <div class="space-y-1">
                            ...
                            <input x-model="form.postal_code" name="postal_code" type="text"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-2 space-y-1">
                            ...
                            <textarea x-model="form.delivery_notes" name="delivery_notes" rows="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input x-model="form.contact_name" name="contact_name" type="text"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">
                        </div>
                        <div class="space-y-1">
                            ...
                            <input x-model="form.contact_phone" name="contact_phone" type="text"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-luffy-red focus:border-transparent transition-all">
                        </div>
                    </div>
            ...
            <div class="lg:col-span-5 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/80 p-6 shadow-sm sticky top-6">
                <div class="mb-4">
                    ...
                    <p class="text-xs text-slate-550 dark:text-slate-400 mt-0.5">Haz clic en el mapa o arrastra el marcador rojo al punto exacto de entrega.</p>
                </div>
        ...
        function addressForm() {
            return {
                ...
                form: {
                    ...
                    reference: @json($address->reference ?? ''),
                    delivery_notes: @json($address->delivery_notes ?? ''),
                    district: @json($address->district ?? ''),
                    province: @json($address->province ?? ''),
                    department: @json($address->department ?? ''),
                    postal_code: @json($address->postal_code ?? ''),
                    country: @json($address->country ?? 'Peru'),
                    contact_name: @json($address->contact_name ?? ''),
                    contact_phone: @json($address->contact_phone ?? ''),
                    latitude: @json($address->latitude ? (string)$address->latitude : '-6.7719'),
                    longitude: @json($address->longitude ? (string)$address->longitude : '-79.8389'),
                    is_default: @json((bool)$address->is_default)
                },
                ...
                init() {
                    const presetTypes = ['home', 'work', 'study'];
                    const currentType = @json($address->address_type ?? 'home');
                    if (presetTypes.includes(currentType)) {
                        this.selected_preset = currentType;
                    } else {
                        this.selected_preset = 'other';
                        this.custom_type = currentType;
                    }

                    // Preselect matching qualified district
                    const currentDist = this.form.district;
                    const currentProv = this.form.province;
                    const currentDept = this.form.department;
                    const matched = this.activeDistricts.find(d => {
                        const parts = d.split('|');
                        if (parts.length === 3) {
                            return parts[2].toLowerCase().trim() === currentDist.toLowerCase().trim() &&
                                   parts[1].toLowerCase().trim() === currentProv.toLowerCase().trim() &&
                                   parts[0].toLowerCase().trim() === currentDept.toLowerCase().trim();
                        }
                        return d.toLowerCase().trim() === currentDist.toLowerCase().trim();
                    });
                    if (matched) {
