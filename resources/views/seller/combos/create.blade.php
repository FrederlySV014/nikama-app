<x-layouts.seller>
    <x-slot:title>Crear Combo - Nikama</x-slot:title>

    <div class="max-w-4xl mx-auto space-y-6" x-data="{ 
        businessId: '{{ old('business_id', '') }}',
        items: {{ json_encode(old('products', [['product_id' => '', 'quantity' => 1]])) }},
        ...
        onBusinessChange() {
            // Reset items when business changes
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-3xl font-bold font-['Outfit'] text-slate-800 dark:text-white mt-2">Nuevo Combo Promocional</h1>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('seller.combos.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex items-center gap-2 pt-8">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-amber-500 border-slate-300 rounded focus:ring-amber-500">
