<x-layouts.seller>
    <x-slot:title>Editar Producto - Nikama</x-slot:title>
 
    <div class="max-w-4xl mx-auto space-y-6" x-data="{ 
        trackStock: {{ old('track_stock', $product->track_stock ? 'true' : 'false') }}, 
        requiresPrep: {{ old('requires_preparation', $product->requires_preparation ? 'true' : 'false') }},
        mainImageName: '',
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-3xl font-bold font-['Outfit'] text-slate-800 dark:text-white mt-2">Editar Producto</h1>
        ...
        <form method="POST" action="{{ route('seller.products.update', $product) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="business_id" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-750 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-colors @error('business_id') border-rose-500 @enderror">
                            @foreach ($businesses as $b)
                                <option value="{{ $b->id }}" {{ old('business_id', $product->business_id) === $b->id ? 'selected' : '' }}>{{ $b->business_name }}</option>
                            @endforeach
                        </select>
                        @error('business_id')
                            <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="category_id" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-750 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-colors @error('category_id') border-rose-500 @enderror">
                            ...
                            @foreach ($categories as $c)
                                <option value="{{ $c->id }}" {{ old('category_id', $selectedCategoryId) === $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                            <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-750 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-colors">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div x-show="trackStock" class="flex items-center gap-2 md:col-span-1 pt-8">
                        <input type="checkbox" name="allow_backorder" id="allow_backorder" value="1" {{ old('allow_backorder', $product->allow_backorder) ? 'checked' : '' }} class="w-4 h-4 text-amber-500 border-slate-300 rounded focus:ring-amber-500">
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm space-y-6">
                ...
                @if ($product->main_image_url)
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-300">Imagen Principal Actual</label>
                        <div class="w-32 h-32 rounded-xl bg-slate-50 dark:bg-slate-900 overflow-hidden border border-slate-200 dark:border-slate-700 relative group">
                            <img src="{{ $product->main_image_url }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                        </div>
                    </div>
                @endif
 
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-50 dark:border-slate-700/30">
                    <!-- Main Image Input -->
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-300 mb-2">Reemplazar Imagen Principal</label>
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-900 hover:bg-slate-100/60 dark:hover:bg-slate-850/60 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-semibold">Cargar nueva imagen principal</p>
                                </div>
                    ...
                    <!-- Additional Images -->
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-300 mb-2">Agregar Más Imágenes a la Galería</label>
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-900 hover:bg-slate-100/60 dark:hover:bg-slate-850/60 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <svg class="w-8 h-8 mb-2 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-semibold">Cargar imágenes adicionales</p>
                                </div>
                ...
                @if ($product->images->isNotEmpty())
                    <div class="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-700/30">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-300">Imágenes de Galería Actuales</label>
                        <div class="grid grid-cols-4 sm:grid-cols-6 gap-4">
                            @foreach ($product->images as $img)
                                <div class="relative w-full aspect-square rounded-xl bg-slate-50 dark:bg-slate-900 overflow-hidden border border-slate-200 dark:border-slate-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-750 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-colors @error('status') border-rose-500 @enderror">
                            ...
                            <option value="suspended" {{ old('status', $product->status) === 'suspended' ? 'selected' : '' }}>Suspendido</option>
                        </select>
                        @error('status')
                            <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" step="0.1" name="weight_grams" value="{{ old('weight_grams', $product->weight_grams) }}" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-750 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-colors">
                        @error('weight_grams')
                            <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2 flex items-center gap-2">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }} class="w-4 h-4 text-amber-500 border-slate-300 rounded focus:ring-amber-500">
