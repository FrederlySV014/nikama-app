...
<head>
    ...
    <title>Acceso Conductores - Nikama Repartidores</title>
    <!-- Fonts -->
...
<body class="antialiased font-sans flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
    <div class="min-h-screen flex flex-col lg:flex-row">
        ...
        <div class="hidden lg:block lg:w-1/2 h-screen sticky top-0 overflow-hidden bg-slate-100 dark:bg-slate-900">
            <img src="{{ asset('nikama-drivers-auth.jpg') }}" alt="Nikama Repartidores" class="w-full h-full object-cover">
        </div>
            ...
            <div class="w-full max-w-md space-y-8 z-10">
                ...
                <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">
                        Ingresa a tu cuenta para ver tu historial de entregas.
                    </p>
                </div>
                ...
                <!-- Formulario -->
                <form class="space-y-6" action="{{ route('driver.login.post') }}" method="POST">
                    @csrf
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input id="email" name="email" type="email" autocomplete="email" required value="{{ old('email') }}"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent transition-all"
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <div class="flex items-center">
                            <input id="remember" name="remember" type="checkbox"
                                class="h-4 w-4 rounded border-gray-300 text-slate-500 focus:ring-slate-500 cursor-pointer">
                            <label for="remember" class="ml-2 block text-slate-700 dark:text-slate-300 font-medium cursor-pointer">Recordarme</label>
                        </div>

                        <div class="text-sm">
                            <a href="#" class="font-bold text-slate-500 hover:text-slate-600 transition">¿Olvidaste tu contraseña?</a>
                        </div>
                    </div>
                ...
                <div class="text-center text-sm">
                    <span class="text-slate-600 dark:text-slate-400">¿Quieres ser repartidor?</span>
                    <a href="{{ route('driver.register') }}" class="font-bold text-slate-700 hover:text-slate-600 transition ml-1">Regístrate y postula aquí</a>
                </div>
